import sys
import input_parser as ip
from emitter import Emitter
from receiver import Receiver
from mirror import Mirror
from laser_circuit import LaserCircuit

'''
Name:   Emily Lewis Dando
SID:    500687002
Unikey: elew6213

run - Runs the entire program. It needs to take in the inputs and process them
into setting up the circuit. The user can specify optional flags to perform
additional steps, such as -RUN-MY-CIRCUIT to run the circuit and -ADD-MY-MIRRORS
to include mirrors in the circuit.

You are free to add more functions, as long as you aren't modifying the
existing scaffold.
'''


def is_run_my_circuit_enabled(args: list[str]) -> bool:
    # only requires implementation once you reach RUN-MY-CIRCUIT
    '''
    Returns whether or not '-RUN-MY-CIRCUIT' is in args.
    
    Parameters
    ----------
    args - the command line arguments of the program
    '''
    i = 0
    while i < len(args):
        if args[i] == "-RUN-MY-CIRCUIT":
            return True
        i += 1
    return False


def is_add_my_mirrors_enabled(args: list[str]) -> bool:
    # only requires implementation once you reach ADD-MY-MIRRORS
    '''
    Returns whether or not '-ADD-MY-MIRRORS' is in args.
    
    Parameters
    ----------
    args - the command line arguments of the program
    '''
    i = 0
    while i < len(args):
        if args[i] == "-ADD-MY-MIRRORS":
            return True
        i += 1
    return False


def initialise_circuit() -> LaserCircuit:
    # only requires implementation once you reach GET-MY-INPUTS
    '''
    Gets the inputs for the board size, emitters and receivers and processes
    it to create a LaserCircuit instance and return it. You should be using
    the functions you have implemented in the input_parser module to handle
    validating each input.

    Returns
    -------
    A LaserCircuit instance with a width and height specified by the user's
    inputted size. The circuit should also include each emitter and receiver
    the user has inputted.
    '''
    #create circuit
    print("Creating circuit board...")
    valid_size = False
    while valid_size == False:
        size = input("> ")
        if ip.parse_size(size) is not None:
            circuit = LaserCircuit(ip.parse_size(size)[0], ip.parse_size(size)[1])
            print(f"{circuit.width}x{circuit.height} board created.\n")
            valid_size = True

    #add emitters
    print("Adding emitter(s)...")
    i = 0
    while i < 10:
        em = input("> ")
        if em == "END EMITTERS":
            break

        else:
            if ip.parse_emitter(em) != None and circuit.valid_emitter(ip.parse_emitter(em)) != False:
                circuit.add_emitter(ip.parse_emitter(em))
                i += 1

    print(f"{i} emitter(s) added.\n")

    #add receivers
    print("Adding receiver(s)...")
    i = 0
    while i < 10:
        re = input("> ")
        if re == "END RECEIVERS":
            break
        else:
            if ip.parse_receiver(re) is not None and circuit.valid_receiver(ip.parse_receiver(re)) != False:
                if circuit.add_receiver(ip.parse_receiver(re)) == True:
                    i += 1
    print(f"{i} receiver(s) added.")

    #circuit.print_board()

    return circuit

    

def set_pulse_sequence(circuit: LaserCircuit, file_obj) -> None:
    # only requires implementation once you reach RUN-MY-CIRCUIT
    '''
    Handles setting the pulse sequence of the circuit. 
    The lines for the pulse sequence will come from the a file named
    /home/input/<file_name>.in. 
    You should be using the functions you have implemented in the input_parser module 
    to handle validating lines from the file.

    Parameter
    ---------
    circuit - The circuit to set the pulse sequence for.
    file_obj - A file like object returned by the open()
    '''
    
    try:
        with open(file_obj, 'r') as fobj:
            print("Setting pulse sequence...")
            line = fobj.readline()
            i = 1
            while line:
                if get_unset_symbols(circuit) == "":
                    break
                if line.strip(" ") == "":
                    continue
                else:
                    print(f"-- ({get_unset_symbols(circuit)})")
                    print(f"Line {i}: {line.strip()}")
                    seq = ip.parse_pulse_sequence(line)

                    if seq is not None:
                    #unset emitters not none
                        emitter = emitter_to_set(circuit, seq)

                        if emitter is not None:
                            emitter.set_pulse_sequence(seq[1], seq[2])
                        elif existing_emitter(circuit, seq) == True:
                            print(f"Error: emitter '{seq[0]}' already has its pulse sequence set")
                        else:
                            print(f"Error: emitter '{seq[0]}' does not exist")
                    i += 1
                    line = fobj.readline()
            if get_unset_emitters(circuit) != []:
                print("Please update input to include a pulse sequence for each emitter.")
            else:
                print("Pulse sequence set.")
    except FileNotFoundError:
        print("Error: -RUN-MY-CIRCUIT flag detected but /home/input/pulse_sequence.in does not exist")
        quit()

def get_unset_symbols(lasercircuit: LaserCircuit):
    i = 0
    symbols = ""
    emitters = get_unset_emitters(lasercircuit)
    while i<len(emitters):
        symbols += f"{emitters[i].symbol}, "
        i += 1
    return symbols.rstrip(', ')

def get_unset_emitters(lasercircuit: LaserCircuit): #-> list(emitters)
    i = 0
    unset = []
    emitters = lasercircuit.get_emitters()
    while i < len(emitters):
        if emitters[i].is_pulse_sequence_set() == False:
            unset.append(emitters[i])
        i += 1
    return unset

def emitter_to_set(lasercircuit: LaserCircuit, pulseseq): #-> emitter
    i = 0
    unset = get_unset_emitters(lasercircuit)
    while i < len(unset):
        if pulseseq[0] == unset[i].symbol:
            return unset[i]
        i += 1
    return None

def existing_emitter(lasercircuit, pulseseq):#-> bool
    i = 0
    emitters = lasercircuit.get_emitters()
    while i < len(emitters):
        if pulseseq[0] == emitters[i].symbol:
            return True
        i += 1
    return False

def add_mirrors(circuit: LaserCircuit) -> None:
    # only requires implementation once you reach ADD-MY-MIRRORS
    '''
    Handles adding the mirrors into the circuit. You should be using the
    functions you have implemented in the input_parser module to handle
    validating each input. 
    
    Parameters
    ----------
    circuit - the laser circuit to add the mirrors into
    '''
    print("Adding mirror(s)...")
    i = 0
    end = False
    while end == False:
        mirror = input("> ")
        if mirror == "END MIRRORS":
            end = True
        else:
            if ip.parse_mirror(mirror) is not None:
                mirror = ip.parse_mirror(mirror)
                if circuit.valid_mirror(mirror) != False:
                    if circuit.add_mirror(mirror) == True:
                        i += 1
    print(f"{i} mirror(s) added.")


def main(args: list[str]) -> None:
    # only requires implementation once you reach GET-MY-INPUTS
    # will require extensions in RUN-MY-CIRCUIT and ADD-MY-MIRRORS
    '''
    Responsible for running all code related to the program.

    Parameters
    ----------
    args - the command line arguments of the program
    '''

    #remove the line below once you start implementing this function
    lasercircuit = initialise_circuit()
    print("")
    if is_add_my_mirrors_enabled(args) == True:
        print("<ADD-MY-MIRRORS FLAG DETECTED!>\n")
        add_mirrors(lasercircuit)
        print("")
        lasercircuit.print_board()
        print("")
    else:
        lasercircuit.print_board()
        print("")
    if is_run_my_circuit_enabled(args) == True:
        print("<RUN-MY-CIRCUIT FLAG DETECTED!>\n")
        set_pulse_sequence(lasercircuit, '/home/input/pulse_sequence.in')
        lasercircuit.run_circuit()



if __name__ == '__main__':
    '''
    Entry point of program. We pass the command line arguments to our main
    program. We do not recommend modifying this.
    '''
    main(sys.argv)

